// Browser-friendly version of server/src/game/timeShiftEngine.js
// Requires `window.Chess` from chess.js.
// Exposes `window.TimeShiftEngine`.
 
(function () {
  function requireChess() {
  // Try common globals from different chess.js builds
  if (typeof window.Chess === 'function') return window.Chess;                 // old UMD
  if (typeof window.Chess === 'object' && typeof window.Chess.Chess === 'function') return window.Chess.Chess;
  if (typeof window.chess === 'object' && typeof window.chess.Chess === 'function') return window.chess.Chess;
  if (typeof window.ChessJS === 'function') return window.ChessJS;
 
  throw new Error('chess.js not loaded: expected global Chess (or Chess.Chess / chess.Chess)');
}
  ...
  class TimeShiftEngine {
    ...
    applyMove({ boardIndex, from, to, promotion = 'q' }) {
      ...
      if (move.flags && String(move.flags).includes('e')) {
      ...
      if (!move.castle && move.flags) {
        if (String(move.flags).includes('k')) move.castle = 'kingside';
        if (String(move.flags).includes('q')) move.castle = 'queenside';
      }
      if (move.castle) {
        let rookFrom;
        let rookTo;
      ...
      if (typeof game.isCheckmate === 'function' ? game.isCheckmate() : game.in_checkmate && game.in_checkmate()) {
  ...
  window.TimeShiftEngine = TimeShiftEngine;
})();